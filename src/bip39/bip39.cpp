#include "../../include/cardanoplusplus/bip39/bip39.hpp"

namespace Cardano{

static std::uint16_t * word2bits(char const * const mnemotic, char const diccionary[][2048], std::size_t * const word_bits_length) noexcept //suport at 255 word, limited to 24 word (128 - 256 bits)
{
    ...
    *word_bits_length = 0;
    ...
    {
        if(mnemotic[mnemotic_len] == 0x20 || mnemotic[mnemotic_len] == '\0' ) //si encuentra un espacio lo registra
        {
            if(mnemotic[mnemotic_len] == '\0') //si llega al final de la cadena se sale del while
            {
                while_true = false;
            }
            inicio = fin;
    ...
    if(*word_bits_length < 12 || *word_bits_length > 24 || *word_bits_length%3 != 0){ // condicion de seguridad, si las palabras no estan en el rango
        return nullptr;
    }

    std::uint16_t *word2bits_out = new (std::nothrow) std::uint16_t[*word_bits_length](); // #include <new>, alternativa con new, cambiar free() por delete[]
    //std::uint16_t *word2bits_out = static_cast<std::uint16_t*>(std::calloc(*word_bits_length,sizeof(std::uint16_t))); ///se asigna el espacio de memoria requerido para almacenar los bits
    *word_bits_length = 0;
            ...
            if(mnemotic[mnemotic_len] == 0x20 || mnemotic[mnemotic_len] == '\0' ) //si encuentra un espacio lo registra
            {
                ...
                if((fin - inicio) > 2) //comprueba si el espacio es mayor a 2 para considerarlo una palabra
                {
                    ...
                    if((fin - inicio) < 9){ //para que no se genere un desbordamiento en el buffer buffer_word
                        ...
                        for(std::uint16_t bits = 0; bits < 2047; bits++) //se busca el nombre en el diccionario dado
                        {
                            if(std::strcmp(buffer_word, diccionary[bits]) == 0){
                                word2bits_out[*word_bits_length] = bits;
                                *word_bits_length += 1;
                                word_is_found = true;
                                break;
                            }
                    ...
                    if(word_is_found == false ){ //en caso de que no encuentre la palabra en el diccionario;
                        sodium_memzero(word2bits_out, *word_bits_length);/// Pone a Cero la Memoria antes de liberarla
...
{
    std::size_t word_bin_length;
    std::uint8_t *entropy = nullptr;
    std::uint16_t *word_bin = word2bits(mnemotic, diccionary, &word_bin_length);

    if(entropy_len != nullptr && word_bin != nullptr){ // aqui se comienza a reconstruir la entropia con la numeracion las palabras
        *entropy_len = (word_bin_length * 44) / 33; //se pasan la cantidad de bytes que tiene la entropy
        entropy = new (std::nothrow) std::uint8_t[*entropy_len](); // #include <new>, alternativa con new, cambiar free() por delete[]
        //entropy = static_cast<std::uint8_t*>(std::calloc(*entropy_len, sizeof(std::uint8_t))); ///se asigan un espacio de memoria a entropy
        if(entropy != nullptr){ //sigue solo si se puede asignar memoria
            ...
            for(std::uint8_t i=0; i < word_bin_length; i++){
                cbw = 11;
                if(entropy_count < *entropy_len){ //para evitar desbordamiento de memoria en entropy[]
                    while(cbw != 0){
                        if(cbe <= cbw){
                            ...
                            if(entropy_count >= *entropy_len){ //se activa en caso de que entropy_count se salga de los limites de entropy[]
            ...
            std::uint8_t cs_length = static_cast<std::uint8_t>((*entropy_len * 8) / 32);
            std::uint8_t checksumsha256[crypto_hash_sha256_BYTES]; //almacena el hash de la entropia
            crypto_hash_sha256(checksumsha256, entropy, *entropy_len); ///se obtiene el hash sha256 con la libreria libsodium, iniciar antes con sodium_init()
            ...
            if(word_bin_cs_bits != cs_bits){ ///Se comparan los cs_bits de ambos para ver si la entropia es replicable (valida)
                sodium_memzero(entropy, *entropy_len); /// Pone a Cero la Memoria
                //free(entropy);
                delete[] entropy;
                entropy = nullptr;
            }
        }
        sodium_memzero(word_bin, word_bin_length);/// Pone a Cero la Memoria antes de liberarla
        delete[] word_bin;
        //std::free(word_bin);
    }
    return entropy;
...
char * entropy2mnemotic(std::uint8_t const * const entropy, std::size_t const *const entropy_len, char const diccionary[][2048], std::size_t * const mnemotic_len) noexcept
{
    ...
    if(entropy_len != nullptr && *entropy_len < 256){  //comprueba que el largo de la entropia no exceda los 255 bytes
        entropy_length_bits = static_cast<std::uint16_t>((*entropy_len) * 8); // Se convierte de bytes a bits la longitud de la entropia (rango 128 - 256 bits)
    }
    ...
    for(std::uint8_t i = 0; i <= ms_length; i++){ //recorre todas las palabras
        ...
        while(bt > 0){
            if(bt >= pbi_e){  // Si bt es mayor o igual a los bit existentes a tomar de la entropia, se desplaza bits a la izquierda
                ...
                if(pbe >= static_cast<std::uint8_t>(*entropy_len)){ // si se excede el limite del array de la entropia se adicionan los bits de cs_bits
    ...
    std::size_t words_length = (ms_length * 9);
    if(mnemotic_len != nullptr){
        *mnemotic_len = words_length;
    }
    char *words = new (std::nothrow) char[words_length](); // #include <new>, alternativa con new, cambiar free() por delete[]
