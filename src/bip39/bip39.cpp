...
std::uint8_t * mnemotic2entropy(char const * const mnemotic, char const diccionary[][2048], std::size_t * const entropy_length_bytes) noexcept
{
    ...
    if(entropy_length_bytes != nullptr && word_bin != nullptr){ // aqui se comienza a reconstruir la entropia con la numeracion las palabras
        *entropy_length_bytes = (word_bin_length * 44) / 33; //se pasan la cantidad de bytes que tiene la entropy
...
char * entropy2mnemotic(std::uint8_t const * const entropy, std::size_t const *const entropy_length_bytes, char const diccionary[][2048], std::size_t * const mnemotic_length) noexcept
{
    ...
    if(entropy_length_bytes != nullptr && *entropy_length_bytes < 256){  //comprueba que el largo de la entropia no exceda los 255 bytes
        entropy_length_bits = static_cast<std::uint16_t>((*entropy_length_bytes) * 8); // Se convierte de bytes a bits la longitud de la entropia (rango 128 - 256 bits)
    }
    ...
    for(std::uint8_t i = 0; i <= ms_length; i++){ //recorre todas las palabras
        ...
        while(bt > 0){
            if(bt >= pbi_e){  // Si bt es mayor o igual a los bit existentes a tomar de la entropia, se desplaza bits a la izquierda
                ...
                if(pbe >= static_cast<std::uint8_t>(*entropy_length_bytes)){ // si se excede el limite del array de la entropia se adicionan los bits de cs_bits
